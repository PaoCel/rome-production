...
export default function OptionCard({
  ...
  return (
    ...
    >
      {/* Cover */}
      <button onClick={onOpen} className="relative block aspect-[4/3] w-full overflow-hidden bg-surface-2">
        {cover?.type === 'image' ? (
          <img
      ...
      <div className="flex flex-1 flex-col p-3.5">
        <button onClick={onOpen} className="text-left">
          <h3 className="break-words font-semibold text-ink group-hover:text-brand-700">
            {title}
          </h3>
        </button>
        {requirementName && (
          <p className="mt-0.5 truncate text-xs text-faint" title={requirementName}>
            for: {requirementName}
          </p>
        )}
